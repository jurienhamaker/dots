...
const reYesterday =
  /^last seen yesterday at (\d\d?):(\d\d) ([aApP])\.?\s?[mM]\.?$/;
...
function conv12to24(str) {
  ...
  if (hr == 12 && str.match(re)[3].toLowerCase() === "a") {
    hr = 0;
  }
  const add = str.match(re)[3].toLowerCase() === "p" && hr != 12;

  return (
    (hr + 12 * add).toString().padStart(2, "0") +
    ":" +
  ...
  [
    ...
    .forEach((e) => {
      if (e.innerText.length > 0) {
        e.innerText = conv12to24(e.innerText);
      }
    });
}
function changeLastSeenTodayTime() {
  [
    ...
    .forEach((e) => {
      if (e.innerText.length > 0) {
        e.innerText = lastseentodayconv12to24(e.innerText);
      }
    });
}
function changeLastSeenYesterdayTime() {
  [
    ...
    .forEach((e) => {
      if (e.innerText.length > 6) {
        e.innerText = lastseenyesterdayconv12to24(e.innerText);
      }
    });
}

module.exports = (config, Ferdium) => {
  console.log("Time changer inited");
  setInterval(() => {
    console.log("Time changer running");
    changeMessageTime();
